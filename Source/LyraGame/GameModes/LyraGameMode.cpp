...
void ALyraGameMode::BeginPlay()
{
	...
	// Get target actor
	UGameplayStatics::GetAllActorsOfClass(GetWorld(), AFiringRangeDummy::StaticClass(), ObjectiveList);

	// Set seed
	RandomStream.Initialize(42);
	
	if (ObjectiveList.Num() == 0)
	{
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, TEXT("No target actor found"));
	}
}

void ALyraGameMode::NextObjective()
{
	if (ObjectiveList.Num() > 0)
	{
		/*
		 * FMath::RandRange: random actors each time
		 * RandomStream.RandRange: same actors each time
		 * Chose one.
		 */
		
		int32 SelectedActor = FMath::RandRange(0, ObjectiveList.Num() - 1);
		// int32 SelectedActor = RandomStream.RandRange(0, ObjectiveList.Num() - 1);

		// If we don't want the same target selected twice in a row
		while (SelectedActor == LastTarget)
		{
			SelectedActor = FMath::RandRange(0, ObjectiveList.Num() - 1);
			// SelectedActor = RandomStream.RandRange(0, ObjectiveList.Num() - 1);
		}
		
		LastTarget = SelectedActor;
		TargetActor = Cast<AFiringRangeDummy>(ObjectiveList[SelectedActor]);
		// ObjectiveList.RemoveAt(SelectedActor); // If we don't want the same target to be selected again
		TargetLocation = TargetActor->GetTargetPointMesh()->GetComponentLocation();
		TargetActor->RotateTarget(TargetActor->StartRotation, TargetActor->EndRotation);
	}
	else
	{
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, TEXT("No more targets"));
	}
}

void ALyraGameMode::UpdateTargetLocation()
{
	if (TargetActor)
	{
		TargetLocation = TargetActor->GetTargetPointMesh()->GetComponentLocation();
...
void ALyraGameMode::ObjectiveReady()
{
	OnObjectiveReady.Broadcast();
}
// FIRING RANGE-----------------------------------------------------------------------------------------------------------
